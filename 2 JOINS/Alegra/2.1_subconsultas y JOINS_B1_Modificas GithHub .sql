------------------------------------------------------------------------------------------------
-- SELECT con suncolsultas y JOINS_(MODIFICACIONES GITHHUB) ---
------------------------------------------------------------------------------------------------
+/*1
...
*/
select
    components.name "Elemento",
    components.assetidentifier "Código",
    components.serialnumber "Serie",
    components.installatedon "Año Instal",
    components.spaceid,
    spaces.id,
    spaces.name "Aula"
from components
...
where facilityid = 1  
        and (lower(components.name) not like '%Tuberia%'
         or lower(components.name) not like '%Muro%'
         or lower(components.name) not like '%Techo%'
         or lower(components.name) not like '%Suelo%');
...
from
    ...
    from
        components
        join spaces on components.spaceid = space.id
    where
        facilityid = 1
    order by 1 desc)
where
