...
en el facility 1.
*/selec id, warrantystarton, spaceid
from components
where facility = 1;

select count(*), count(warrantystarton), count(id), count(spaceid), max(id),
count(spaceid), count(distinct spaceid),
max(warrantystarton)
from components where facilityid=1;

select count(*) from components where facilityid = 1 and warrantystarton is not null;

select 
    count (*),
    count (id) NumerosComponentes,
